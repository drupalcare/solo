...
((Drupal, once) => {
  ...
  function initializeCustomMultiSelectDualList(customSelect) {
    ...
    // Attach event listener for adding items (Event Delegation for dynamically created elements)
    // Use event delegation to handle both adding and removing in one event listener
    customSelect.addEventListener("click", function (event) {
      const addButton = getActionButton(event.target, '.dual-list-add');
      const removeButton = getActionButton(event.target, '.dual-list-remove');

      if (addButton) {
        const option = addButton.closest('.dual-list-option');
        moveOption(option, availableList, selectedList, true, hiddenSelect);
      }

      if (removeButton) {
        const option = removeButton.closest('.dual-list-option');
    ...
    // Use `once()` to prevent duplicate event bindings for Clear All button
    once('dualListClearAll', clearAllBtn).forEach(() => {
      clearAllBtn.addEventListener("click", function () {
        clearAllSelections(selectedList, availableList, hiddenSelect);
      });
    });

    // Initialize selected values at page load
